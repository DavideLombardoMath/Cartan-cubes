load "FFT.m";
SetLogFile("AbelFFT.out");

// Global variables
// GlobalMax stores the maximum value witnessed so far for the ratio D(s)/\sqrt{ps}
GlobalMax := 0;
// SRD stores the values of the function \sum_{d | m} 1/d
SRD := [Rationals()!0 : i in [1..10^6]];
...
for p in [ j : j in [3..10^6] | IsPrime(j) and j mod 9 in {2,5} ] do
	"p =", p;
	MaxP := 0;        // the maximum ratio D(s)/\sqrt{ps} witnessed so far for the prime p
	...
	*/
	...
	/*
	Next we compute the set of a_2 \in F_p such that 1 + a_2 * SqrEps
	is a cube in F_{p^2}. In the notation of the paper, this is the set F(1)
	*/
	time A1 := [ a2 : a2 in GF(p) | (1+a2*SqrEps)^(Integers()!( (p^2-1)/3 ) ) eq 1 ];
	/*
	In general, the set of a_2 such that k + a_2 \varepsilon is a cube,
	that is, the set F(k), is simply given by {k*a : a in A1}
	*/
	...
	*/
	...
	= \sum_{a in F(b)} exp(2 pi i / p * a) = c(b)
	*/
	...
	*/
	...
	for s in [1..(p-1)] do
		...
		*/
		...
		/*
		Compute the partial sum defining D(s) and update the maximum
		ratio D(s)/\sqrt{sp}, both for this single p and for all p
		tested so far
		*/
	...
	"Maximum ratio D(s)/(\sqrt{ps}) =", MaxP;    // print the optimal constant for p
